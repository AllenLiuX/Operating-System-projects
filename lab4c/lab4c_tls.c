...
void init_ssl(){
	...
	if (new_context == NULL) {
    	error2("error: cannot get ssl context.");
	}
	ssl = SSL_new(new_context);
	if (ssl == NULL) {
    	error2("Error: cannot do ssl setup.");
	}
	if (!SSL_set_fd(ssl, sock)) {
    	error2("Error: cannot setup fd with ssl.");
	}
	if (SSL_connect(ssl) != 1) {
    	error2("Error: cannot connect ssl.");
	}
}
