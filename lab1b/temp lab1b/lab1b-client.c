	...
	while(1){
		if (poll(poll_fds, 2, 0) > 0) {
			...
			if (stdin_revents == POLLIN) {
				char buff[256];
				int num = read(STDIN_FILENO, &buff, 256);
				for (i = 0; i < num; i++) {
				...
				if(compress_flag){
					...
					write(sockfd, compression_buf, 256 - client_to_server.avail_out);
					if(log_flag){
						char str[20];
						sprintf(str, "%d", 256-client_to_server.avail_out);
						write(log_fd, sent, strlen(sent));
						write(log_fd, str, strlen(str));
						write(log_fd, bytes, strlen(bytes));
						write(log_fd, buff, 256-client_to_server.avail_out);
			...
			if (socket_revents == POLLIN) {
				...
				if(compress_flag){
					...
					server_to_client.next_in = (unsigned char *) buff;
